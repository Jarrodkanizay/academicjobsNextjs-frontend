...
const Job = ({ job }) => {
  ...
  if (location == 'Academic_Hub') {
    content = (
      <article className="media bg-white border p-4 mb-4 rounded-xl shadow-lg" data-id="59972"
      /// onClick={() => {
      /// window.history.pushState({}, "New Page Title", `/academic-hub/${title?.replace(/\W+/g, '-').toLowerCase()}/${id}/`);
      //dispatch(setJob(job))
      ///  }}
      >
